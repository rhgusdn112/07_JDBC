...
public class JDBCExample1 {
	public static void main(String[] args) {
		 ...
		 * 
		 * - Java에서 DB에 연결(접근) 할 수 있게 해주는 Java API (Java에서 제공하는 코드) -> java.sql 패키지에 존재함
		 */
		...
		// == DBeaver에서 사용하는 DB 연결(계정)과 같은 역할의 객체
		...
		try {
			...
			String port = ":1521"; // 프로그램 구분 번호

			String dbName = ":XE"; // DBMS 이름(XE == eXpress Edition)

			String userName = "KH_GHW"; // 사용자 계정명
			...
			// 연결된 DB(Connection)에 SQL을 전달하고 결과를 반ㅇ환받을
			// Statement 객체를 생성함
			...
			/*
			 * 6. 조회 결과가 담겨있는 ResultSet을 커서(Cursor)를 이용해 1행씩 접근해 각 행에 작성된 컬럼값 얻어오기
			 */
			...
			while (rs.next()) {
				...
				System.out.printf("사번 : %s / 이름 : %s / 부서코드 : %s / 직급코드 : %s / 급여 : %d / 입사일 : %s \n ", empId, empName,
						deptCode, jobCode, salary, hireDate.toString());
			}

		} catch (SQLException e) {
			// SQLException : DB연결과 관련된 예외의 최상위 부모
			e.printStackTrace();
